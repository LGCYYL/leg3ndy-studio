...
function showModal(type, title, msg, onConfirm = null) {
    ...
    if (type === 'confirm') {
        ...
        confirmBtn.onclick = () => { modal.classList.add('hidden'); if (onConfirm)
            onConfirm(); };
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (window.electronAPI && window.electronAPI.onUpdateEvent) {
        window.electronAPI.onUpdateEvent((evt) => {
            if (evt.type === 'checking') {
                ...
                if (evt.manual)
                    ...
                    console.error("AutoUpdate Error:", evt.error);
            }
    ...
    window.onfocus = () => { if (!document.getElementById('historyView').classList.contains('hidden'))
        revalidateHistory(); };
});
function showToast(msg, isPermanent = false, showAction = false, type = 'info') {
...
async function loadConfig() {
    try {
        ...
        if (data.download_path) {
            selectedDownloadPath = data.download_path;
            updatePathUI(data.download_path);
        }
    }
    catch (e) { }
}
...
async function toggleAutoStart() {
    ...
    if (window.electronAPI)
        await window.electronAPI.setSysSettings({ openAtLogin: c, startHidden: h });
}
...
async function toggleMinimizeTray() {
    ...
    if (window.electronAPI)
        await window.electronAPI.setSysSettings({ minimizeToTray: c });
}
function startCarousel() {
    const slides = document.querySelectorAll('.slide');
    const root = document.documentElement;
    if (slides.length > 0)
        root.style.setProperty('--ambient-color', slides[0].getAttribute('data-color'));
    let idx = 0;
    setInterval(() => {
...
function navTo(view) {
    ...
    if (view === 'history') {
        loadHistory();
        startLibraryHeartbeat();
    }
    else {
        stopLibraryHeartbeat();
    }
    if (view === 'queue') {
        renderQueue();
        updateEstimatedSize();
...
async function resetPath() {
    ...
    if (data.status === 'ok') {
        ...
        updatePathUI(data.path);
        window.electronAPI.notifyDownloadPathChanged(data.path);
    }
}
// --- BUSCA ---
document.getElementById('btnSearch').addEventListener('click', fetchInfo);
document.getElementById('urlInput').addEventListener('keypress', (e) => { if (e.key === 'Enter')
    fetchInfo(); });
    ...
    try {
        ...
        if (data.error) {
            ...
            if (currentData.type === 'playlist')
                renderPlaylist(currentData);
            else if (currentData.type === 'search_results')
                renderSearchList(currentData);
            else
                renderResult(currentData);
        }
    }
    catch (e) {
        showModal('alert', 'Erro de Conexão', 'Não foi possível conectar ao servidor interno (Backend). Verifique se o aplicativo foi iniciado corretamente.');
    }
    finally {
...
function renderSearchList(data) {
    const inputStr = document.getElementById('urlInput').value;
...
function addPlaylistToQueue() {
    ...
    if (checkboxes.length === 0)
        return showModal('alert', 'Nenhum vídeo selecionado', 'Por favor, marque pelo menos um vídeo ou música da lista para adicionar à fila.');
    checkboxes.forEach(cb => {
        const idx = cb.getAttribute('data-idx');
        const item = currentData.entries[idx];
...
function updateQueueBadge() {
    ...
    document.getElementById('queueCount').innerText = c.toString();
    document.getElementById('queueCount').classList.toggle('hidden', c === 0);
}
function updateEstimatedSize() {
    ...
    let txt = `Tamanho Estimado: ${humanSize(total)}`;
    if (hasEstimates)
        txt += "* (Aprox)";
...
function clearQueue() {
    if (downloadQueue.length === 0)
        return;
    if (isDownloading)
        return showModal('alert', 'Aguarde', 'Você não pode limpar a fila enquanto houver downloads ativos. Por favor, cancele ou aguarde o término.');
...
async function processQueue() {
    ...
    if (pending.length === 0)
        return showModal('alert', 'Sua fila está vazia', 'Adicione vídeos ou músicas à fila antes de iniciar o download.');
    isDownloading = true;
    isCancelled = false;
    toggleButtons(true);
    renderQueue();
    ...
    renderQueue();
    if (!isCancelled)
        new Notification("LEG3NDY Studio", { body: "Todos os downloads da fila foram concluídos com sucesso." });
}
function renderQueue() {
    const list = document.getElementById('queueList');
    if (downloadQueue.length === 0) {
        list.innerHTML = '<div class="empty-state">Sua fila está vazia.</div>';
        return;
    }
    list.innerHTML = '';
...
function toggleButtons(downloading) {
    ...
    if (downloading) {
        ...
        btnClean.classList.add('btn-disabled');
        btnCancel.classList.remove('btn-disabled');
        btnCancel.removeAttribute('disabled');
    }
    else {
        btnStart.classList.remove('btn-disabled');
        btnClean.classList.remove('btn-disabled');
...
async function revalidateHistory(silent = false) {
    ...
    try {
        ...
        if (!silent)
            console.error("Erro no heartbeat da biblioteca");
    }
}
async function loadHistory() {
    const list = document.getElementById('historyList');
    if (!list.hasChildNodes())
        list.innerHTML = '<p style="text-align:center; color:#64748B;">Carregando Biblioteca...</p>';
    try {
        const res = await fetch('http://127.0.0.1:5000/api/library');
        const data = await res.json();
        renderHistoryUI(data);
        ...
        list.innerHTML = '<p style="text-align:center;">Erro ao carregar arquivos locais.</p>';
    }
}
...
function toggleSortDropdown(e) {
    ...
    if (!isOpen) {
        const close = (eClick) => {
            if (!menu.contains(eClick.target)) {
...
function renderHistoryUI(data) {
    ...
    if (!data || data.length === 0) {
        list.innerHTML = '<div class="empty-state">Sua biblioteca está vazia. Os arquivos baixados aparecerão aqui.</div>';
        return;
    }
    ...
    if (list.parentElement)
        list.parentElement.scrollTop = scrollPos;
    ...
    if (window.electronAPI && window.electronAPI.getThumbnail) {
        thumbRequests.forEach(req => {
            window.electronAPI.getThumbnail(req.rawPath).then((thumb) => {
                if (thumb) {
                    _thumbCache.set(req.rawPath, thumb);
                    const el = document.getElementById(`thumb-lib-${req.index}`);
                    if (el)
                        el.innerHTML = `<img src="${thumb}" style="width:100%; height:100%; object-fit:cover; border-radius:12px;">`;
                    const icon = document.getElementById(`hicon-${req.index}`);
                    if (icon)
                        icon.setAttribute('style', 'padding:0; background:none; border:none; width:80px; height:45px;');
                }
            }).catch(() => { });
        });
    }
}
function triggerSmartSync() { if (refreshTimeout)
    clearTimeout(refreshTimeout); refreshTimeout = setTimeout(() => { revalidateHistory(); }, 500); }
function startLibraryHeartbeat() { stopLibraryHeartbeat(); libraryHeartbeat = setInterval(() => { revalidateHistory(true); }, 2000); }
function stopLibraryHeartbeat() { if (libraryHeartbeat) {
    clearInterval(libraryHeartbeat);
    libraryHeartbeat = null;
} }
